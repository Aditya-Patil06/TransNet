...
public class clsClientBookingReport
{
    ...
    // Static method to fetch booking report data for a client
    public static clsClientBookingReport[] getClientBookingReportInformation(int clientID)
    {
        return dlsClient.getClientBookingReportInformation(clientID);  // Delegate to dlsClient
    }

    // Utility method to display a single booking report
    public static void showClientBooking(clsClientBookingReport temp)
    {
    ...
    {
        int testClientID = 1;  // Example client ID for testing
        clsClientBookingReport[] reports = getClientBookingReportInformation(testClientID);

        if (reports != null && reports.length > 0)
        {
            System.out.println("Booking Report for Client ID: " + testClientID);
            System.out.println("-----------------------------");
            for (clsClientBookingReport report : reports)
            {
                showClientBooking(report);
                System.out.println("-----------------------------");
            }
        }
        else
        {
            System.out.println("No booking records found for Client ID: " + testClientID);
        }
    }
}